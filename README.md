...
- **命令行看图**：`ImageViewer.exe "<图片路径>"` 直接打开 jpg/png/bmp；双击 exe（无参数）会弹出图片选择对话框，选中即看图、取消退出。
...
- **文件关联**：首次运行会提示是否设为默认图片查看器；右键菜单也可注册/取消 jpg/png/bmp 的打开候选（机器级 HKLM 需管理员授权）并引导系统默认应用设置。
