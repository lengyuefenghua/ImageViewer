...
namespace ImageViewer.App.Standalone
{
    ...
    public partial class StandaloneViewerWindow : FluentWindow
    {
        ...
        private static string ResolveWindowStatePath()
        {
            try
            {
                return Path.Combine(ImageViewerPaths.ForCurrentUser().ConfigurationDirectory, ViewerWindowStateStore.FileName);
            }
            catch (Exception error)
            {
                Diagnostics.Sink.Log(LogSeverity.Debug, LoggerName, "解析查看器窗口状态路径失败", error);
                return null;
            }
        }
