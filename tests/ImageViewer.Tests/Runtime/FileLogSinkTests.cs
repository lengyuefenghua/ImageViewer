...
namespace ImageViewer.Tests.Runtime
{
    public sealed class FileLogSinkTests : IDisposable
    {
        ...
        {
            directory = Path.Combine(Path.GetTempPath(), "idv-filelog-" + Guid.NewGuid().ToString("N"));
        ...
        {
            var sink = new FileLogSink(directory, LogSeverity.Debug);
        ...
        // 文件日志每次写入即关闭文件；这里做带总超时的条件轮询，避免磁盘延迟导致偶发失败。
